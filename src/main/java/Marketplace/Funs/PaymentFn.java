...
//    ========================================================================
    ...
    private void onProcessPayment(Context context, Message message) {
        ...
        paymentAsyncTaskState_.addInvoice(order.getId(), invoice);
...
//        );
    ...
    private void onStockFinalChangeDone(Context context, Message message) {
        ...
        paymentAsyncTaskState.addCompletedSubTask(orderId);
        if (paymentAsyncTaskState.isAllSubTaskCompleted(orderId)) {
            String log_ = getPartionText(context.self().id())
                    + "receive all sub tasks stockFinalChange response for order: " + orderId + "\n";
            showLogPrt(log_);

            paymentAsyncTaskState.removeTask(orderId);

            Invoice invoice = paymentAsyncTaskState.getInvoice(orderId);
            Order order = invoice.getOrder();
            long customerId = order.getCustomerId();
            paymentAsyncTaskState.removeSingleInvoice(orderId);

            if (paymentResv.getOrderStatus() == Enums.OrderStatus.PAYMENT_SUCCESS) {

//                long orderPartition = orderId % Constants.nOrderPartitions;
                ...
                sendMessage(context, CustomerFn.TYPE, String.valueOf(customerId),
                        NotifyCustomer.TYPE,
            ...
            String log__ = getPartionText(context.self().id())
                    + "payment all finished, message sent for order, customer and shipment: " + orderId + "\n";
            showLog(log__);
        }
        context.storage().set(PAYMENT_ASYNC_TASK_STATE, paymentAsyncTaskState);
    }
